...
let get_env cfg =
  ...
  in
  let vars_edge = G.fold_edges_e (fun (_, (stmts, _), _) l -> (collect_vars_stmt_seq stmts) @ l) cfg [] in
  let vars = Array.map Var.of_string (Array.of_list (List.sort_uniq compare (vars_vertex @ vars_edge))) in
